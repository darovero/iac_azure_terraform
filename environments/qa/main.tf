...
module "automation_accounts" {
  ...
  identity_ids = length(each.value.managed_identity_keys) > 0 ? toset([
    for identity_key in each.value.managed_identity_keys :
    module.managed_identities[identity_key].managed_identity_id
  ]) : null
